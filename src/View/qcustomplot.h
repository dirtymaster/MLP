...
enum Interaction {
    ...
    ,
    iSelectPlottablesBeyondAxisRect = 0x100  ///< <tt>0x100</tt> When performing plottable selection/hit
...
void QCPDataContainer<DataType>::performAutoSqueeze() {
                                   ...
                                   // memory, below 1k points don't even bother
    {
        ...
        shrinkPreAllocation = mPreallocSize > usedSize * 1.5;  // preallocation can grow into postallocation,
                                                               // so can be smaller
    }
...
ItemType *QCustomPlot::itemAt(const QPointF &pos, bool onlySelectable) const {
    ...
    foreach (QCPAbstractItem *item, mItems) {
        ...
        if (!currentItem ||
            (onlySelectable && !currentItem->selectable()))  // we could have also passed onlySelectable
                                                             // to the selectTest function, but checking
                                                             // here is faster, because we have access to
...
public:
    ...
    */
    ...
    virtual double selectTest(
        ...
        QVariant *details = 0) const;  // actually introduced in QCPLayerable as non-pure, but we want
